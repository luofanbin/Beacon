...
typedef union
{
    int nResult[DICOM_GSDF_GRAYSCALE_NUMBER-1];
    double dbLuminace[DICOM_GSDF_GRAYSCALE_NUMBER*3];
}UnTg18TypeResult;
typedef struct
{
    EmReportType emReportType;
    EmTg18Type emType;
...
typedef struct _ST_QA_CHECK_CONDITION_
{
    ...
    void InitialReportType(EmReportType emType)
    {
         switch (emType)
         {
            case REPORT_DIN:
             dbUniformDevation=25;
             break;
         default:
             dbUniformDevation=30;
             break;
...
typedef struct _ST_QA_RESULT_ :public stSqlOperatorBase
{
    ...
    int nLuminanceDeviationDataTwo;
     _ST_QA_RESULT_()
    {
...
stSqlOperatorBase::intialPointer();
     ...
     _ST_QA_RESULT_& operator=(const _ST_QA_RESULT_& data)
     {
         ...
         this->nTestPrerequisites=data.nTestPrerequisites;
         ...
         this->nLuminanceDeviationDataTwo=data.nLuminanceDeviationDataTwo;
     }
...
{
    CURVE_GAMMA=0,//标准的gamma曲线类型 调背光时背光只设最前面一组
    CURVE_DICOM, //标准的Dicom曲线标准  调背光时带环境光的背光只设Dicom
    CURVE_CIE,	 //标准的CIE曲线标准
    CURVE_USE_DEFINE_GUID,
    CURVE_ONLY_BRIGHT, //DICOM shares a Lut,only calibrated bright
    CURVE_NATIVE, //Lut Bypass only calibrated bright
    CURVE_HYBIRD, //for all Bypass
...
typedef struct _VISUAL_EVALUATION_DATA_
{
    ...
    int ViasulEvaluationIndex[11];
    int PixelFaultsData[5];
    int SelectRkIndex;
...
typedef struct
{
    ...
    Luminance_Deviation_Data LuminanceDeviationInfoTwo;
}Display_Data;
...
typedef struct
{
    QString strDisplayType;
    QString SN;
    int nUpdateCount;
}StUploadDisplayRecords;
...
typedef struct _STRUCT_LUT_ATTRIBUTE_
{
    BYTE btLutID;
    emCurveType emCurveValue;
    DWORD btBackLight;
    double dbValue;
    double dbMaxLuminance;
    DWORD dwContrast;
    _STRUCT_LUT_ATTRIBUTE_()
    {
        btLutID=0;
        emCurveValue=CURVE_GAMMA;
        btBackLight=0;
        dbValue=0.0;
        dbMaxLuminance=0.0;
        dwContrast=0;
...
typedef struct _ST_CALI_BRIGHT_DATA_
{
    double dbLuminSt;
    double btBrightness;
    double dbLuminCur;
    QMap<int,double> mapBrightLumiTable;
    bool blIsInternetSensor;

    _ST_CALI_BRIGHT_DATA_()
    {
        dbLuminSt=0.0;
        btBrightness=0.0;
        dbLuminCur=0.0;
        blIsInternetSensor=false;
    }
}stCaliBrightData;
...
typedef struct
{
    BYTE btModeType;
    bool blIsColor;
    QString strPanelType;
    bool blInternalSensor;
    bool blAmbientSensor;
    bool blBacklightSensor;
    double dbReflectionFactor;
    BYTE btLutDataFormat;
    unsigned short dwLutSize;
    unsigned short dwDicomSuport;
    unsigned short dbLuminanceMaxSuport;
}stGeneralInfo;

typedef struct
{
    unsigned short nLutCountReal;
    unsigned short nLutValueMaxReal;
    unsigned short nLutValueMaxDeal;
    unsigned short nLutCountDeal;
}stLutParamerter;
...
typedef enum
{
    STANDARD_CALIBRATION_TYPE,
    COSTOM_CALIBRATION_TYPE,
    AUTO_COSTOM_CALIBRATION_TYPE
}EmCalibrationType;

typedef enum
{
    //CALIBRATION_CURRENT_GAMMA,
    CALIBRATION_CURRENT_COLOR,
    CALIBRATION_ALL_COLOR
}EmCalibrationMode;

typedef enum
{
    COMMUNICATION_DDCCI,
    COMMUNICATION_USBHID
}CommunicationType;
...
typedef struct
{
    ...
    int nRemoteTaskId;
    int nDisplayIndex;
    QString strDisplayMode;
    QString strDisplaySN;
    QString strTaskType;
    QDateTime dtDueDateTime;
    QDateTime dtDealTime;
    QString strTaskIntervalTime;
    bool blRepeat;
    bool blOnOff;
    bool blStatus;
    bool blDelete;
    bool blAutomaticCalibration;
    bool blIsRemote;
}StTaskListInfo;

typedef enum
{
    QUERY_ALL_UNFINISHED_TASK,
    QUERY_SINGLE_UNFINISHED_TASK,
    QUERY_SINGLE_TASK_ID,
    QUERY_SINGLE_TASK_REMOTE_ID,
    QUERY_SINGLE_CABRETION_TASK_ID
}QueryTaskType;
...
{
   SELECT_FILE_OK,
   FILE_NAME_INCONFORMITY,
   NULL_FILE,
   FILE_CONTENT_ERR,
   DATA_INCOMPLETE,
   DATA_EXCEEDED,
   GAMMA_NAME_DISACCORD,
   INVALID_DATA,
   MISSING_GAMMA_TABLE,
   FILE_CONTENT_OK
}enCheckFileErr;
...
{
    BYTE btColorIndex;
    double dbMaxLuminace;
    double dbMinLuminace;
}StColorCompareData;
...
extern int g_nSelectRKType;
extern bool g_blInternalVersion;
