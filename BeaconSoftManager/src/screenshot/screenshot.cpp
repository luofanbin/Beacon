...
void ScreenShot::loadBackgroundPixmap(QPoint mousePosition,bool isMouseMove)
{
    ...
    saveRectList.clear();
    //SaveTestLogInfo("==================Screenshot==================\n");
    for(int i=0;i<nMonitorNum;i++)
    {
        currRect=deskTop->screenGeometry(i);
                //GetScreenRect(i);
        //QString str=QString("Display %1:%2 %3 %4 %5\n").arg(i).arg(currRect.x()).arg(currRect.y()).arg(currRect.width()).arg(currRect.height());
        //SaveTestLogInfo(str);
                //GetScreenRect(i);
        saveRectList.append(currRect);
        startPoint.setY(qMin(startPoint.y(),currRect.y()));
        startPoint.setX(qMin(startPoint.x(),currRect.x()));
    ...
    m_nScreenX=startPoint.x();
    m_nScreenY=startPoint.y();
...
//    }
    ...
    this->setGeometry(startPoint.x(),startPoint.y(),n_right-n_left+1,n_bottom-n_top+1);
    QScreen * screen = QGuiApplication::primaryScreen();
    m_loadPixmap = screen->grabWindow(QApplication::desktop()->winId(),startPoint.x(),startPoint.y(),n_right-n_left+1,n_bottom-n_top+1); //抓取当前屏幕的图片;
    m_screenwidth = m_loadPixmap.width();
    m_screenheight = m_loadPixmap.height();
    resize(n_right-n_left+1,n_bottom-n_top+1);
    move(QPoint(startPoint.x(),startPoint.y()));
    update();
}
...
//    {
//        QRect temRect=GetScreenRect(m_nCurrMonitor);
//        selectedRect=temRect;
//    }
...
void ScreenShot::mousePressEvent(QMouseEvent *event)
{
    ...
    {
        m_stretchRectState = getStrethRectState(pos);
        ...
        {
            // 是否在选中的矩形中;
            if (isPressPointInSelectRect(pos))
            {
                if(!m_bIsStartPaint)
                {
                ...
                if(!m_bIsPaintMousePress)
                {
                    ...
                    else
                    {
                        if(m_textEdit->toPlainText().size())
                        {
                            ...
                            texts="";
                            QString qstrPlainText = m_textEdit->toPlainText();
                        ...
                        int n_x=(m_endPoint.x()-m_beginPaintPoint.x())<90?(m_beginPaintPoint.x()-(m_endPoint.x()-90)):0;
                        int n_y=(m_endPoint.y()-m_beginPaintPoint.y())<40?(m_beginPaintPoint.y()-(m_endPoint.y()-40)):0;
                        if(n_x==0)
                            nMaxTextWidth=m_endPoint.x()-m_beginPaintPoint.x();
                        else
                            nMaxTextWidth=90;

                        if(n_y==0)
                            nMaxTextHeight=m_endPoint.y()-m_beginPaintPoint.y();
                        else
                            nMaxTextHeight=40;

                        m_beginPaintPoint=event->pos();

                        m_beginPaintPoint.setX(m_beginPaintPoint.x()-n_x);
                        m_beginPaintPoint.setY(m_beginPaintPoint.y()-n_y);
...
void ScreenShot::mouseMoveEvent(QMouseEvent *event)
{
    ...
    for(int i=0;i<nMonitorNum;i++)
    {
        ...
        if((pos.x()>=currRect.x()) && ((pos.x())<=currRect.right())
          && ((pos.y())>=currRect.y())  && ((pos.y())<=currRect.bottom()) && (m_nCurrMonitor!=i))
        {
            m_nCurrMonitor=i;
            update();
        }
    ...
    {
       m_endMovePoint =pos;
       // m_endMovePoint = QPoint(m_nScreenX+event->pos().x(),m_nScreenY+event->pos().y());
        update();
    }
    ...
    else if(m_paintState != NotStartPaint && m_bIsPaintMousePress)
    {
        ...
        {
            if(curLines.isEmpty())
                m_beginPaintPoint=QPoint(abs(m_beginPaintPoint.x()-m_nScreenX),abs(m_beginPaintPoint.y()-m_nScreenY));
        ...
        else
        {
            if(isPressPointInSelectRect(pos))
                m_endPaintPoint = pos;
            //m_endPaintPoint = pos;
            //m_endPaintPoint = QPoint(m_nScreenX+event->pos().x(),m_nScreenY+event->pos().y());
            update();
        }
    }

    // 根据鼠标是否在选中区域内设置鼠标样式;
    StretchRectState stretchRectState = getStrethRectState(pos);
    if (stretchRectState != NotSelect)
    {
        setStretchCursorStyle(stretchRectState);
    }
    else if (isPressPointInSelectRect(pos))
    {
    ...
    else if (!isPressPointInSelectRect(pos) && m_currentCaptureState != BeginMoveCaptureArea)
    {
...
void ScreenShot::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if(event->button() == Qt::LeftButton)
    {
        qDebug()<<"m_currentCaptureState"<<m_currentCaptureState;
        if (m_currentCaptureState == BeginCaptureImage)
        {
...
void ScreenShot::paintEvent(QPaintEvent *event)
{
    ...
    for(int i=0;i<nMonitorNum;i++)
    {
        ...
        if(m_nCurrMonitor==i)
        {
            ...
            m_painter.drawRect(abs(currRect.x()-m_nScreenX),abs(currRect.y()-m_nScreenY),currRect.width()-3,currRect.height()-3);
            m_currentSelectRect=QRect(abs(currRect.x()-m_nScreenX),abs(currRect.y()-m_nScreenY),currRect.width(),currRect.height());
        }
        else
            m_painter.fillRect(QRect(abs(currRect.x()-m_nScreenX),abs(currRect.y()-m_nScreenY),currRect.width(),currRect.height()), shadowColor);
    }
    ...
    {
        QRect cRect=QRect(abs(deskTop->screenGeometry(m_nCurrMonitor).x()-m_nScreenX),abs(deskTop->screenGeometry(m_nCurrMonitor).y()-m_nScreenY),
                          deskTop->screenGeometry(m_nCurrMonitor).width(),deskTop->screenGeometry(m_nCurrMonitor).height());
        m_painter.fillRect(cRect, shadowColor);
        m_currentSelectRect = getSelectRect();
        m_currentSelectRect=QRect(abs(m_currentSelectRect.x()-m_nScreenX),abs(m_currentSelectRect.y()-m_nScreenY),m_currentSelectRect.width(),m_currentSelectRect.height());
        m_painter.fillRect(m_currentSelectRect,QColor(0, 0, 0, 0));
        double db_sc_x=(double)deskTop->screenGeometry(m_nCurrMonitor).width()/deskTop->screenGeometry(m_nCurrMonitor).width();
        double db_sc_y=(double)deskTop->screenGeometry(m_nCurrMonitor).height()/deskTop->screenGeometry(m_nCurrMonitor).height();
        drawCaptureImage(db_sc_x,db_sc_y);
        break;
    }
    }
    ...
    if(m_paintState != NotStartPaint)
    {
        ...
        rectTem = getPaintRect();
        rectTem.moveTo(abs(rectTem.x()-m_nScreenX),abs(rectTem.y()-m_nScreenY));
        switch (m_paintState)
        {
        case Arrows:
            drawArrow(QPoint(abs(m_beginPaintPoint.x()-m_nScreenX),abs(m_beginPaintPoint.y()-m_nScreenY)),
                      QPoint(abs(m_endPaintPoint.x()-m_nScreenX),abs(m_endPaintPoint.y()-m_nScreenY)));
            break;
        case Ellipses:
            m_painter.drawEllipse(rectTem);
            break;
        case Rectangles:
            m_painter.drawRect(rectTem);
            break;
        case Lines:
    ...
    if(m_bIsAddToContainer)
    {
        ...
        case Arrows:
        {
            PC.beginPoint = QPoint(abs(m_beginPaintPoint.x()-m_nScreenX),abs(m_beginPaintPoint.y()-m_nScreenY));
            PC.endPoint = QPoint(abs(m_endPaintPoint.x()-m_nScreenX),abs(m_endPaintPoint.y()-m_nScreenY));
        }
            break;
        case Ellipses:
        {
...
void ScreenShot::onSaveButtonClicked()
{
    m_textEdit->hide();//隐藏文本框，防止文本框出现在图片中
    QRect rect = getSelectRect();
    rect.moveTo(abs(rect.x()-m_nScreenX),abs(rect.y()-m_nScreenY));
    QPixmap capture = this->grab(rect);
    //获取当前年月日及时间，作为图片的默认名
    QDate date = QDate::currentDate();
    QTime time = QTime::currentTime();
    QString strDefaultSaveName;
    //年-月-日_时-分-秒.png
...
void ScreenShot::onTextChanged()
{
    ...
    //if (m_textEdit->toPlainText().size() < 10)
    //{
    //    m_textEdit->resize(m_textEdit->toPlainText().size() * 10 + 50, 40);
    //}
    //else
    //{
        //m_plainTextEdit->resize(sqrt(m_plainTextEdit->toPlainText().size()) * 15 + 30, m_plainTextEdit->document()->size().rheight() + 10);
        int n_width=0,n_height=50;
        //if((m_textEdit->toPlainText().size() * 15 + 20)<nMaxTextWidth)
        if((m_textEdit->document()->size().rwidth()+20)<nMaxTextWidth)
            n_width=m_textEdit->document()->size().rwidth()+20;
                    //m_textEdit->toPlainText().size() * 15 + 20;
           // m_textEdit->resize(m_textEdit->toPlainText().size() * 15 + 20, m_textEdit->document()->size().rheight() + 10);
        else
            n_width=nMaxTextWidth;

        if(n_width<90)
            n_width=90;

            if((m_textEdit->document()->size().rheight() + 50)<nMaxTextHeight)
                n_height=m_textEdit->document()->size().rheight() + 50;
            else
                n_height=nMaxTextHeight;

       m_textEdit->resize(nMaxTextWidth, n_height);
    //}




//    if(maxsize*(textFontSize)<90 && counts*(textFontSize+15)<40)
//    {
//            m_textEdit->resize(90,40);
//    }
//    else
...
//        if(((maxsize+1)*(textFontSize)+50)<nMaxTextWidth)
//            n_width=(maxsize+1)*(textFontSize)+50;
//        else
//        {
//            n_width=nMaxTextWidth;
//            if(((maxsize+1)*(textFontSize)+50)%nMaxTextWidth==0)
//                counts=(((maxsize+1)*(textFontSize)+50)/nMaxTextWidth);
//            else
//                counts=(((maxsize+1)*(textFontSize)+50)/nMaxTextWidth)+1;
//        }

//        if((counts*(textFontSize+15)+50)<nMaxTextHeight)
//            n_height=counts*(textFontSize+15)+50;
//        else
//            n_height=nMaxTextHeight;
//            m_textEdit->resize(n_width,n_height);
//    }
}
...
QRect ScreenShot::getSelectRect()
{
    ...
    if (m_currentCaptureState == BeginCaptureImage || m_currentCaptureState == FinishCaptureImage)
    {
        ...
        {
            if(m_beginPoint==m_endPoint)
            {
                QRect temRect=deskTop->screenGeometry(m_nCurrMonitor);
                m_beginPoint=temRect.topLeft();
                m_endPoint=temRect.bottomRight();
                return deskTop->screenGeometry(m_nCurrMonitor);
            }
        }
...
// 绘制当前选中的截图区域;
void ScreenShot::drawCaptureImage(double dbScX,double dbScY)
{
    QRect rect1=m_currentSelectRect;
    rect1.setWidth(rect1.width()*dbScX);
    rect1.setHeight(rect1.height()*dbScY);
...
QRect ScreenShot::getMoveRect()
{
    ...
    qDebug()<<"m_beginPoint:"<<m_beginPoint<<"m_endPoint:"<<m_endPoint;
...
QPoint ScreenShot::getMovePoint()
{
    ...
    //移动选区是否超出屏幕左边界;
    if (currentRect.topLeft().x() + movePoint.x() < m_nScreenX)
    {
        movePoint.setX(0 - currentRect.topLeft().x());
    }
    //移动选区是否超出屏幕上边界;
    if (currentRect.topLeft().y() + movePoint.y() < m_nScreenY)
    {
...
{
    int posX, posY;
    QPoint topLeftPoint = QPoint(abs(QCursor::pos().x()-m_nScreenX),abs(QCursor::pos().y()-m_nScreenY));
    ...
    // 绘制放大图;
    QPixmap endPointImage = m_loadPixmap.copy(QRect(abs(QCursor::pos().x()-m_nScreenX) - 15, abs(QCursor::pos().y()-m_nScreenY) - 11, 30, 22)).scaled(END_POINT_RECT_WIDTH, END_POINT_IMAGE_HEIGHT);
    ...
    QColor endPointColor = image.pixel(QPoint(abs(QCursor::pos().x()-m_nScreenX),abs(QCursor::pos().y()-m_nScreenY)));
...
void ScreenShot::moveFunKey()
{
    ...
    {//功能栏移动
        m_currentSelectRect = getSelectRect();//更新位置，防止功能栏与截图区域移动不同步
        qDebug()<<"m_currentSelectRect:"<<m_currentSelectRect;
        m_currentSelectRect.moveTo(abs(m_currentSelectRect.x()-m_nScreenX),abs(m_currentSelectRect.y()-m_nScreenY));
        if(m_currentSelectRect.bottom() <= m_screenheight - 90)
        {
            ...
            QRect temRect=deskTop->screenGeometry(m_nCurrMonitor);
            if(m_currentSelectRect.height()>(temRect.height()-90))
            {
